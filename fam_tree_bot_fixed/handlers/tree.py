...
async def tree(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as session:
        ...
        def node(uid: int, db_user=None) -> dict:
            return {
                "name":  (db_user.first_name if db_user else str(uid))[:16],
                "title": "",
                "color": (db_user.profile_color if db_user else "blue"),
            }
        ...
        for rel in rels:
            ...
            if rel.relation_type == RelationType.SPOUSE:
                ...
                if rel.user_id == user.user_id:      # user est le PARENT
                    child_nodes.append(n)
                else:                                  # user est l'ENFANT
...
async def bigtree(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Pour bigtree on fait un arbre texte récapitulatif de toutes les familles
    async with AsyncSessionLocal() as session:
