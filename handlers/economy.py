    ...
    async with AsyncSessionLocal() as session:
        ...
        if req_type_str == "marry":
            ...
            text = (f"💒 {mention(sender)} et {mention(target)} sont maintenant maries ! 🎉\n"
                    ...
                    f"💑 Vous pouvez créer un compte commun avec /couple create")
...
async def divorce(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Dissoudre le compte commun si existant
        from database.db import dissolve_couple_account, get_couple_account
        account = await get_couple_account(session, user.user_id)
        couple_msg = ""
        if account:
            share = account.balance // 2
            await dissolve_couple_account(session, user.user_id, spouse_id)
            if share > 0:
                couple_msg = (
                    f"\n💰 Compte commun dissous : {_fmt(account.balance)} $ partagés "
                    f"({_fmt(share)} $ chacun)."
                )
            else:
                couple_msg = "\n💳 Compte commun dissous (solde vide)."
        await remove_relationship(session, user.user_id, spouse_id, RelationType.SPOUSE)
    await update.message.reply_text(
        f"💔 {mention(user)} et {mention(spouse)} ont divorce.{couple_msg}",
        parse_mode=ParseMode.HTML,
    )
