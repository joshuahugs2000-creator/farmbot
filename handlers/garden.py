...
def _plant_status(g: Garden) -> str:
    ...
    if now >= ready_at:
        return f"  Slot {g.slot+1}: {emoji} {g.plant_type} — ✅ Prêt à récolter ! (`/harvest {g.slot+1}`)"
...
async def garden(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not settings.garden_enabled:
            return await update.message.reply_text("🌿 Le jardin est désactivé dans ce groupe.")
        plants  = await get_garden(session, user.user_id, group_id)
        u       = await get_user(session, user.user_id)
        coins   = u.coins if u else 0

    used_slots = {g.slot for g in plants}
    free       = GARDEN_SLOTS - len(used_slots)

    lines = [f"🌱 <b>Jardin de {update.effective_user.first_name}</b>  💰 {coins} coins\n"]
    for slot_i in range(GARDEN_SLOTS):
        g = next((p for p in plants if p.slot == slot_i), None)
        if g:
            lines.append(_plant_status(g))
        else:
            lines.append(f"  Slot {slot_i+1}: 🟫 Vide  (`/plant {slot_i+1} <plante>`)")
...
async def plant_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Usage : /plant <slot 1-5> <type>"""
    if not is_group(update):
        return await update.message.reply_text("❗ Commande de groupe uniquement.")
    if not context.args or len(context.args) < 2:
        return await update.message.reply_text(
            f"Usage : /plant <slot 1-{GARDEN_SLOTS}> <{'|'.join(PLANT_TYPES.keys())}>"
        )
    try:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if any(g.slot == slot for g in existing):
            return await update.message.reply_text("❗ Ce slot est déjà occupé. Récoltez d'abord.")
    ...
    await update.message.reply_text(
        f"{info['emoji']} {plant_type} planté dans le slot {slot+1} !\n"
        f"⏳ Prêt dans {hours}h{mins:02d}m — 💰 Valeur : {info['value']} coins"
    )


async def harvest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Usage : /harvest <slot> ou /harvest (récolte tout)"""
    if not is_group(update):
        return await update.message.reply_text("❗ Commande de groupe uniquement.")
    ...
    async with AsyncSessionLocal() as session:
        ...
        for g in plants:
            grow_time = PLANT_TYPES[g.plant_type]["grow_time"]
            if now >= g.planted_at + timedelta(seconds=grow_time):
                ready.append(g)

        # Filtrer par slot si précisé
        if context.args:
            try:
                slot = int(context.args[0]) - 1
        ...
        if not ready:
            return await update.message.reply_text("🌿 Rien à récolter pour l'instant !")
